...
from notesApp import create_account_spanish, delete_account_spanish, delete_spanish, forgot_password_spanish, home_spanish, login_spanish, options_spanish
from notesApp import search_results_spanish, update_profile_spanish
...
@flask_obj.route('/login_spanish', methods=['POST'])
def handle_login():
    # Check username and password
    email = request.form.get('email')
    password = request.form.get('password')
    ...
    # Hardcoded username and password for demonstration
    if not bool(db.session.execute(db.select(User).where(User.email == email)).scalar()):
            return render_template('login_spanish.html', error1='No existe una cuenta con este correo electrónico.') 
    ...
    if check_password_hash(db.session.execute(db.select(User.password).where(User.email == email)).scalar(), password):
        ...
        # Redirect to home.html upon successful login
        return redirect('home_spanish.html')
    else:
        # You can handle authentication failure here
        return render_template('login_spanish.html', error2='Contraseña incorrecta')

@flask_obj.route('/create_account_spanish', methods=['GET', 'POST'])
def create_account():
    data = request.form
    if request.method == 'POST':
        # Get user input from the registration form
        ...
        # Check if the email is already associated with an existing account
        if bool(db.session.execute(db.select(User).where(User.email == email)).scalar()):
            return render_template('create_account_spanish.html', data=request.form, error='existe una cuenta con este correo electrónico.')
        ...
        # Redirect to the login page after successful registration
        return redirect('login_spanish.html')
    return render_template('create_account_spanish.html', data=data)

@flask_obj.route('/forgot_password_spanish', methods=['GET', 'POST'])
def forgot_password():
    if request.method == 'POST':
        # Check if the provided email exists in the database
        email = request.form.get('email')
        ...
        if user:
            ...
            # Handle the case where the email is not found in the database
            return render_template('forgot_password_spanish.html', error='Email no encontrado.')

    return render_template('forgot_password_spanish.html')
...
# serve home.html to frontend and populate fields with all notes associated with user id
@flask_obj.route('/home_spanish', methods=['GET', 'POST'])
def home():
    note_tuples = []
    # view only notes for specific user
    notes = Note.query.filter(Note.user_id == login_session['id']).all()
    for note in notes:
        note_tuples.append((note.id, note.title, note.body, note.thumb_url))
    return render_template('home_spanish.html', **locals())
...
# redirects the user to a page where notes can be deleted
@flask_obj.route('/delete_spanish', methods=["GET"])
def go_to_delete():
    ...
    for note in notes:
        note_tuples.append((note.id, note.title, note.body))
    return render_template("delete_spanish.html", note_tuples=note_tuples)
...
@flask_obj.route('/options_spanish')
def options():
    return render_template('options_spanish.html')

@flask_obj.route('/delete_account_spanish', methods=['DELETE'])
def delete_account():
    user_to_delete = User.query.filter(User.id == login_session['id']).first()
    try:
        # delete all of user's created notes in the db
        ...
        db.session.delete(user_to_delete)
        db.session.commit()
        flash("Eliminación de usuario exitosa", 'Éxito')
        return jsonify({'status': 'Éxito'})
    
    except:
        db.session.rollback()  # Rollback changes in case of an exception
        flash("Hubo un problema al eliminar al usuario. Por favor, inténtalo nuevamente.", 'Fallo')
        return jsonify({'status': 'Fallo'})
